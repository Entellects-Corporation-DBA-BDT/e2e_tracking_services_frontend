...
export default function PermissionMatrix(){
 ...
 const save=async()=>{setSaving(true);setMessage("");try{const r=await savePositionPermissions(position,rows);setMessage(r.message);window.dispatchEvent(new CustomEvent("e2e-permissions-changed"))}catch(e){setMessage(e?.response?.data?.message||"Permissions could not be saved.")}finally{setSaving(false)}};
 ...
 <div className="permission-role"><label>Position<select value={position} onChange={e=>setPosition(e.target.value)}>{positions.map(p=><option key={p.id} value={p.id}>{p.position_name}</option>)}</select></label><p>Saved changes are applied automatically to active users.</p></div>{message&&<div className="permission-message">{message}</div>}
